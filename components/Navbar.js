...
const NavLinks = ({ onClick }) => (
  <>
    <Link href="#projects" onClick={onClick} className="text-white hover:text-[var(--color-brand-gold)] transition-colors duration-200">
      Projects
    </Link>
    <Link href="#about" onClick={onClick} className="text-white hover:text-[var(--color-brand-gold)] transition-colors duration-200">
      About
    </Link>
    <Link href="#contact" onClick={onClick} className="text-white hover:text-[var(--color-brand-gold)] transition-colors duration-200">
      Contact
    </Link>
  </>
);
  ...
  return (
    <nav className="w-full border-b border-gray-800 bg-black px-6 h-20 sticky top-0 z-50">
      <div className="max-w-7xl mx-auto flex items-center justify-between h-full">
        {/* Logo */}
        <Link href="/" className="text-[var(--color-brand-gold)] text-1x1 font-bold tracking-wide hover:opacity-90">
          10X Freelancer
        </Link>
